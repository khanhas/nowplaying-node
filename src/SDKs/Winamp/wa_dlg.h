...
LRESULT WADlg_handleDialogMsgs(HWND hwndDlg, UINT uMsg, WPARAM wParam,
                               LPARAM lParam); //
void WADlg_DrawChildWindowBorders(
    HWND hwndDlg, int *tab,
    int tabsize); // each entry in tab would be the id | DCW_*

HBITMAP WADlg_getBitmap();

/// define WA_DLG_IMPLEMENT in one of your source files before including this .h
// if you are making a media library plugin, you dont need to do this, look at
// view_ex for an example of how to get the function *'s via an IPC message.
#ifdef __cplusplus
}
#endif
...
int WADlg_initted() { return !!wadlg_bitmap; }

int WADlg_getColor(int idx) {
  if (idx < 0 || idx >= WADLG_NUM_COLORS)
    return 0;
  return wadlg_colors[idx];
}

HBITMAP WADlg_getBitmap() { return wadlg_bitmap; }
...
void WADlg_init(HWND hwndWinamp) // call this on init, or on WM_DISPLAYCHANGE
{
  ...
  wadlg_bitmap =
      (HBITMAP)SendMessage(hwndWinamp, WM_WA_IPC, 0, IPC_GET_GENSKINBITMAP);
  if (wadlg_bitmap) {
...
LRESULT WADlg_handleDialogMsgs(HWND hwndDlg, UINT uMsg, WPARAM wParam,
  ...
  if (uMsg == WM_DRAWITEM) {
    ...
    if (di->CtlType == ODT_BUTTON) {
      ...
      StretchBlt(di->hDC, r.left + 4, r.top, r.right - r.left - 4 - 4, 4, hdc,
                 4, yoffs, 47 - 4 - 4, 4, SRCCOPY); // top center
      BitBlt(di->hDC, r.right - 4, r.top, 4, 4, hdc, 47 - 4, yoffs,
             SRCCOPY); // top right

      StretchBlt(di->hDC, r.left, r.top + 4, 4, r.bottom - r.top - 4 - 4, hdc,
                 0, 4 + yoffs, 4, 15 - 4 - 4, SRCCOPY); // left edge
      StretchBlt(di->hDC, r.right - 4, r.top + 4, 4, r.bottom - r.top - 4 - 4,
                 hdc, 47 - 4, 4 + yoffs, 4, 15 - 4 - 4, SRCCOPY); // right edge

      // center
      StretchBlt(di->hDC, r.left + 4, r.top + 4, r.right - r.left - 4 - 4,
                 r.bottom - r.top - 4 - 4, hdc, 4, 4 + yoffs, 47 - 4 - 4,
                 15 - 4 - 4, SRCCOPY);

      BitBlt(di->hDC, r.left, r.bottom - 4, 4, 4, hdc, 0, 15 - 4 + yoffs,
             SRCCOPY); // bottom left
      StretchBlt(di->hDC, r.left + 4, r.bottom - 4, r.right - r.left - 4 - 4, 4,
                 hdc, 4, 15 - 4 + yoffs, 47 - 4 - 4, 4,
                 SRCCOPY); // bottom center
      BitBlt(di->hDC, r.right - 4, r.bottom - 4, 4, 4, hdc, 47 - 4,
             15 - 4 + yoffs, SRCCOPY); // bottom right

      // draw text
      SetBkMode(di->hDC, TRANSPARENT);

      // this will do a different style for the button text depending on enabled
      // state of the button
      COLORREF colour = wadlg_colors[WADLG_BUTTONFG];
      if (!IsWindowEnabled(di->hwndItem)) {
        COLORREF fg = wadlg_colors[WADLG_WNDFG], bg = wadlg_colors[WADLG_WNDBG];
      ...
      if (GetFocus() == di->hwndItem) {
        ...
        WADlg_dotLine(di->hDC, r.left + 2, r.bottom - 3, r.right - r.left - 3,
                      0);
  ...
  case WM_CTLCOLOREDIT: {
    int bgcolor = (uMsg == WM_CTLCOLOREDIT || uMsg == WM_CTLCOLORLISTBOX)
                      ? wadlg_colors[WADLG_ITEMBG]
                      : (uMsg == WM_CTLCOLORBTN ? wadlg_colors[WADLG_ITEMBG]
                                                : wadlg_colors[WADLG_WNDBG]);
    LOGBRUSH lb = {BS_SOLID, GetNearestColor((HDC)wParam, bgcolor)};
    if (wadlg_lastbrush)
      DeleteObject(wadlg_lastbrush);
    wadlg_lastbrush = CreateBrushIndirect(&lb);
    SetTextColor((HDC)wParam, uMsg == WM_CTLCOLORSTATIC
                                  ? wadlg_colors[WADLG_WNDFG]
                                  : wadlg_colors[WADLG_ITEMFG]);
...
static int RectInRect(RECT *rect1, RECT *rect2) {
  ...
  return ((rect1->top >= rect2->top && rect1->top <= rect2->bottom) ||
          ...
          (rect2->bottom >= rect1->top &&
           rect2->bottom <= rect1->bottom)) // vertical intersect
         && ((rect1->left >= rect2->left && rect1->left <= rect2->right) ||
             (rect1->right >= rect2->left && rect1->right <= rect2->right) ||
             (rect2->left >= rect1->left && rect2->left <= rect1->right) ||
             (rect2->right >= rect1->left &&
              rect2->right <= rect1->right)) // horiz intersect
      ;
}

static void WADlg_removeFromRgn(HRGN hrgn, int left, int top, int right,
                                int bottom) {
...
void WADlg_DrawChildWindowBorders(HWND hwndDlg, int *tab, int tabsize) {
    ...
    if (RectInRect(&ps.rcPaint, &r)) {
      if ((a & 0xffff0000) == DCW_SUNKENBORDER) {
        ...
        if (r.right - r.left < r.bottom - r.top) // vertical
        {
          ...
          if (hrgn)
            WADlg_removeFromRgn(hrgn, left, r.top, left + 1, r.bottom);
        } else // horiz
        {
          ...
          if (hrgn)
            WADlg_removeFromRgn(hrgn, r.left, top, r.right, top + 1);
        }
      }
    }
  }
  ...
  if (hrgn) {
    // erase bkgnd while clipping out our own drawn stuff (for flickerless
    // display)
