...
**
** When you use SendMessage(hwnd_winamp,WM_WA_IPC,(parameter),IPC_*) and specify
*a IPC_*
** which is not currently implemented/supported by the Winamp version being used
*then it
** will return 1 for 'result'. This is a good way of helping to check if an api
*being
...
#define WINAMP_VERSION_MINOR(winampVersion)                                    \
  (winampVersion &                                                             \
   0x000000FF) // returns, i.e. 0x12 for 5.12 and 0x10 for 5.1...

#define IPC_GETVERSION 0
/* int version = SendMessage(hwnd_winamp,WM_WA_IPC,0,IPC_GETVERSION);
**
** The version returned will be 0x20yx for Winamp 2.yx.
** Versions previous to Winamp 2.0 typically (but not always) use 0x1zyx
*for 1.zx.
...
typedef struct {
  ...
  const char *title;
  int length;
} enqueueFileWithMetaStruct; // send this to a IPC_PLAYFILE in a non
                             // WM_COPYDATA,
// and you get the nice desired result. if title is NULL, it is treated as a
// "thing", otherwise it's assumed to be a file (for speed)
...
#define IPC_PLAYFILE                                                           \
  100 // dont be fooled, this is really the same as enqueufile
#define IPC_ENQUEUEFILE 100
#define IPC_PLAYFILEW 1100
#define IPC_ENQUEUEFILEW 1100
/* This is sent as a WM_COPYDATA with IPC_PLAYFILE as the dwData member and the
 *string
 ** of the file / playlist to be enqueued into the playlist editor as the lpData
 *member.
 ** This will just enqueue the file or files since you can use this to enqueue a
 *playlist.
 ...
 **
 **
 ** With 2.9+ and all of the 5.x versions you can send this as a normal
 *WM_WA_IPC
 ** (non WM_COPYDATA) with an enqueueFileWithMetaStruct as the param.
 ** If the title member is null then it is treated as a "thing" otherwise it
 *will be
...
** You should not need to use IPC_DELETE_INT since it is used internally by
*Winamp when
...
** Sending this will start playback and is almost the same as hitting the play
*button.
** The IPC_STARTPLAY_INT version is used internally and you should not need to
*use it
** since it won't be any fun.
*/

#define IPC_CHDIR 103
/* This is sent as a WM_COPYDATA type message with IPC_CHDIR as the dwData value
 *and the
...
** This api can return two different sets of information about current playback
*status.
**
** If mode = 0 then it will return the position (in ms) of the currently playing
*track.
** Will return -1 if Winamp is not playing.
**
** If mode = 1 then it will return the current track length (in seconds).
** Will return -1 if there are no tracks (or possibly if Winamp cannot get the
*length).
**
...
** This api sets the current position (in milliseconds) for the currently
*playing song.
** The resulting playback position may only be an approximate time since some
*playback
** formats do not provide exact seeking e.g. mp3
** This returns -1 if Winamp is not playing, 1 on end of file, or 0 if it was
*successful.
*/
...
**
** IPC_WRITEPLAYLIST will write the current playlist to
*'<winampdir>\\Winamp.m3u' and
** will also return the current playlist position (see IPC_GETLISTPOS).
**
** This is kinda obsoleted by some of the newer 2.x api items but it still is
*good for
** use with a front-end program (instead of a plug-in) and you want to see what
*is in the
** current playlist.
**
** This api will only save out extended file information in the #EXTINF entry if
*Winamp
** has already read the data such as if the file was played of scrolled into
*view. If
** Winamp has not read the data then you will only find the file with its
*filepath entry
...
** It will not change playback status or anything else. It will just set the
*current
...
** after using IPC_SETPLAYLISTPOS then Winamp will start playing the file at
*'position'.
*/
...
#define IPC_GETVOLUME(hwnd_winamp)                                             \
  SendMessage(hwnd_winamp, WM_WA_IPC, -666, IPC_SETVOLUME)
/* (requires Winamp 2.0+)
...
**
** If you pass 'panning' as -666 to this api then it will return the current
*panning.
...
** IPC_GETLISTLENGTH returns the length of the current playlist as the number of
*tracks.
*/
...
** IPC_GETLISTPOS returns the current playlist position (which is shown in the
*playlist
** editor as a differently coloured text entry e.g is yellow for the classic
*skin).
**
** This api is a lot like IPC_WRITEPLAYLIST but a lot faster since it does not
*have to
...
/* (requires Winamp 2.4+)
** This is sent as a WM_COPYDATA using IPC_ADDBOOKMARK as the dwData value and
*the
** directory you want to change to as the lpData member. This will add the
*specified
...
**
**
** In Winamp 5.0+ we use this as a normal WM_WA_IPC and the string is null
*separated as
...
** Note that using this message in this context does not actually add the
*bookmark.
** Do not use, it is essentially just a notification type message :)
*/

#define IPC_INSTALLPLUGIN 130
/* This is not implemented (and is very unlikely to be done due to safety
 *concerns).
 ** If it was then you could do a WM_COPYDATA with a path to a .wpz and it would
 *then
...
** If this fails to make Winamp start after closing then there is a good chance
*one (or
** more) of the currently installed plugins caused Winamp to crash on exit
*(either as a
** silent crash or a full crash log report before it could call itself start
*again.
*/
...
** This is useful for when you're an output plugin and you want to see if the
*stop/close
** happening is a full stop or if you are just between tracks. This returns non
*zero if
...
** IPC_INETAVAILABLE will return 1 if an Internet connection is available for
*Winamp and
...
** IPC_UPDTITLE will ask Winamp to update the information about the current
*title and
** causes GetFileInfo(..) in the input plugin associated with the current
*playlist entry
** to be called. This can be called such as when an input plugin is buffering a
*file so
...
** IPC_REFRESHPLCACHE will flush the playlist cache buffer and you send this if
*you want
** Winamp to go refetch the titles for all of the entries in the current
*playlist.
**
** 5.3+: pass a wchar_t * string in wParam, and it'll do a strnicmp() before
*clearing the cache
*/
...
** Sending this message with 0xdeadbeef as the param will disable all winamp
*windows and
** any other values will enable all of the Winamp windows again. When disabled
*you won't
** get any response on clicking or trying to do anything to the Winamp windows.
*If the
...
/* int (*httpRetrieveFileW)(HWND hwnd, char *url, wchar_t *file, wchar_t
 * *dlgtitle); */
...
** IPC_MBOPEN will open a new URL in the minibrowser. if url is NULL, it will
*open the Minibrowser window.
*/
...
** moves where winamp expects the Options menu in the main menu. Useful if you
*wish to insert a
...
#define IPC_GET_EXTENDED_FILE_INFO                                             \
  290 // pass a pointer to the following struct in wParam
#define IPC_GET_EXTENDED_FILE_INFO_HOOKABLE 296
/* (requires Winamp 2.9+)
** to use, create an extendedFileInfoStruct, point the values filename and
*metadata to the
** filename and metadata field you wish to query, and ret to a buffer, with
*retlen to the
** length of that buffer, and then
*SendMessage(hwnd_winamp,WM_WA_IPC,&struct,IPC_GET_EXTENDED_FILE_INFO);
...
#define IPC_GET_BASIC_FILE_INFO                                                \
  291 // pass a pointer to the following struct in wParam
typedef struct {
  const char *filename;

  int quickCheck; // set to 0 to always get, 1 for quick, 2 for default (if 2,
                  // quickCheck will be set to 0 if quick wasnot used)
...
#define IPC_GET_BASIC_FILE_INFOW                                               \
  1291 // pass a pointer to the following struct in wParam
typedef struct {
  const wchar_t *filename;
...
#define IPC_GET_EXTLIST                                                        \
  292 // returns doublenull delimited. GlobalFree() it when done. if data is 0,
      // returns raw extlist, if 1, returns something suitable for
      // getopenfilename
...
#define IPC_SET_EXTENDED_FILE_INFO                                             \
  294 // pass a pointer to the a extendedFileInfoStruct in wParam
/* (requires Winamp 2.9+)
** to use, create an extendedFileInfoStruct, point the values filename and
*metadata to the
** filename and metadata field you wish to write in ret. (retlen is not used).
*and then
** SendMessage(hwnd_winamp,WM_WA_IPC,&struct,IPC_SET_EXTENDED_FILE_INFO);
** returns 1 if the metadata is supported
** Call IPC_WRITE_EXTENDED_FILE_INFO once you're done setting all the metadata
*you want to update
*/
...
#define IPC_FORMAT_TITLE_EXTENDED                                              \
  298 // similiar to IPC_FORMAT_TITLE, but falls back to Winamp's %tags% if your
      // passed tag function doesn't handle it
typedef struct {
  const wchar_t *filename;
  int useExtendedInfo; // set to 1 if you want the Title Formatter to query the
                       // input plugins for any tags that your tag function
                       // fails on
  ...
  wchar_t *(*TAGFUNC)(const wchar_t *tag,
                      void *p); // return 0 if not found, -1 for empty tag
...
** int (*uncompress)(unsigned char *dest, unsigned long *destLen, const unsigned
*char *source, unsigned long sourceLen);
** right out of zlib, useful for decompressing zlibbed data.
** if you pass the parm of 0x10100000, it will return a wa_inflate_struct * to
*an inflate API.
*/
...
typedef struct _prefsDlgRec {
  ...
  intptr_t
      where; // section in the treelist the prefs page is to be added to
  ...
  intptr_t _id;
  struct _prefsDlgRec *next; // no longer implemented as a linked list, now used
                             // by Winamp for other means
} prefsDlgRec;
...
**
** IPC_ADD_PREFS_DLG:
** To use this you need to allocate a prefsDlgRec structure (either on the heap
*or with
** some global data but NOT on the stack) and then initialise the members of the
*structure
...
*prefs page
** though you shouldn't really ever have to do this but it's good to clean up
*after you
...
*or a
** &prefsDlgRec of the preferences page to open and this is normally done if you
*are
...
** Passing -1 for param will open the preferences dialog to the last page
*viewed.
**
...
*next call
** the prefs dialog would be brought to the front but the page would not be
*changed to the
** specified.
...
*not
** guaranteed since it depends on the order in which the plugins are loaded
*which can
** change on different systems.
**
** Global Hotkeys, Jump To File, Media Library (under General Preferences and
*child pages),
** Media Library (under Plugins), Portables, CD Ripping and Modern Skins are
*custom pages
...
** char
**plugdir=(char*)SendMessage(hwnd_winamp,WM_WA_IPC,0,IPC_GETPLUGINDIRECTORY);
** This returns a pointer to the directory where Winamp has its plugins stored
*and is
** useful if you want store config files in plugins.ini in the plugins folder or
*for
...
** This returns a pointer to the directory where winamp.m3u (and winamp.m3u8 if
*supported) is stored in.
*/

#define IPC_GETM3UDIRECTORYW 338
/* (requires Winamp 5.3+)
** wchar_t
**m3udirW=(wchar_t*)SendMessage(hwnd_winamp,WM_WA_IPC,0,IPC_GETM3UDIRECTORYW);
** This returns a pointer to the directory where winamp.m3u (and winamp.m3u8 if
*supported) is stored in.
*/
...
/* (requires Winamp 5.0+)
** HGLOBAL hglobal =
*(HGLOBAL)SendMessage(hwnd_winamp,WM_WA_IPC,(WPARAM)(HWND)parent,IPC_OPENURLBOX);
** You pass a hwnd for the dialog to be parented to (which modern skin support
*uses).
** This will return a HGLOBAL that needs to be freed with GlobalFree() if this
*worked.
*/
...
** Pass 'parent' as the window which will be used as the parent for a number of
*the built
** in Winamp dialogs and is useful when you are taking over the whole of the UI
*so that
** the dialogs will not appear at the bottom right of the screen since the main
*winamp
** window is located at 3000x3000 by gen_ff when this is used.  Call this again
*with
...
** HWND hwndParent = SendMessage(hwnd_winamp,WM_WA_IPC,(WPARAM)0,
*IPC_GETDIALOGBOXPARENT);
** hwndParent can/must be passed to all modal dialogs (including MessageBox)
*thats uses winamp as a parent
*/

#define IPC_UPDATEDIALOGBOXPARENT 366
/* (requires Winamp 5.53+)
** if you previous called IPC_SETDIALOGBOXPARENT, call this every time your
*window resizes
*/
...
#define EMBED_FLAGS_NOWINDOWMENU 0x4
  // set this bit to prevent gen_ff from automatically adding your window to the
  // right-click menu

#define EMBED_FLAGS_GUID 0x8
  // (5.31+) call SET_EMBED_GUID(yourEmbedWindowStateStruct, GUID) to define a
  // GUID for this window
...
/* (requires Winamp 2.9+)
** HWND myframe =
*(HWND)SendMessage(hwnd_winamp,WM_WA_IPC,(WPARAM)&wa_wnd,IPC_GET_EMBEDIF);
**
** or
**
** HWND myframe = 0;
...
**   *(void**)&embed =
*(void*)SendMessage(hwnd_winamp,WM_WA_IPC,0,IPC_GET_EMBEDIF);
**   myframe = embed(&wa_wnd);
**
** You pass an embedWindowState* and it will return a hwnd for the frame window
*or if you
...
typedef struct embedEnumStruct {
  int (*enumProc)(embedWindowState *ws,
                  struct embedEnumStruct *param); // return 1 to abort
...
/* (requires Winamp 2.9+)
** int valid =
*SendMessage(hwnd_winamp,WM_WA_IPC,(WPARAM)embedhwnd,IPC_EMBED_ISVALID);
** Pass a hwnd in the wParam to this to check if the hwnd is a valid embed
*window or not.
*/
...
** 1: Conversion started. Status messages will be sent to the specified
*callbackhwnd.
...
typedef struct {
  ...
  // hack alert! you can set destformat[6]=mmioFOURCC('I','N','I',' '); and
  // destformat[7]=(int)my_ini_file; (where my_ini_file is a char*)
  HWND callbackhwnd; // window that will receive the IPC_CB_CONVERT notification
                     // messages
...
** You need to call this when you receive the IPC_CB_CONVERTDONE message or when
*you
...
typedef struct {
  ...
  HWND hwndConfig;
  int extra_data[8];
  // hack alert! you can set extra_data[6]=mmioFOURCC('I','N','I',' '); and
  // extra_data[7]=(int)my_ini_file; (where my_ini_file is a char*)
} convertConfigStruct;
...
typedef struct {
  ...
  int length;
  int force_useformatting; // can set this to 1 if you want to force a url to
                           // use title formatting shit
} waHookTitleStruct;

#define IPC_HOOK_TITLES 850
/* (requires Winamp 5.0+)
** If you hook this message and modify the information then make sure to return
*TRUE.
** If you don't hook the message then make sure you pass it on through the
*subclass chain.
**
** LRESULT CALLBACK WinampWndProc(HWND hwnd, UINT umsg, WPARAM wParam, LPARAM
*lParam)
** {
...
// 0: returns a int export_vu_get(int channel) that returns 0-255 (or -1 for bad
// channel)

#define IPC_ISMAINWNDVISIBLE 900
/* (requires Winamp 5.0+)
** int visible=SendMessage(hwnd_winamp,WM_WA_IPC,0,IPC_ISMAINWNDVISIBLE);
** You send this to Winamp to query if the main window is visible or not such as
*by
** unchecking the option in the main right-click menu. If the main window is
*visible then
...
/* (requires Winamp 5.0+)
** This is sent by gen_ff when a modern skin is being loaded to set the colour
*scheme for
** the playlist editor. When sent numElems is usually 6 and matches with the 6
*possible
** colours which are provided be pledit.txt from the classic skins. The elems
*array is
** defined as follows:
**
** elems = 0  =>  normal text
...
**   if(colStr)
**   {
**      // set or inspect the colours being used (basically for gen_ff's
*benefit)
**   }
** }
*/
...
/* This is sent by the system tray when an event happens (you might want to
 *simulate it).
 **
...
/* Input plugins send this when they are done playing back the current file to
 *inform
...
#define IPC_IS_PLAYING_VIDEO                                                   \
  501 // returns >1 if playing, 0 if not, 1 if old version (so who knows):)
#define IPC_GET_IVIDEOOUTPUT 500 // see below for IVideoOutput interface
#define VIDEO_MAKETYPE(A, B, C, D)                                             \
  ((A) | ((B) << 8) | ((C) << 16) | ((D) << 24))
#define VIDUSER_SET_INFOSTRING 0x1000
#define VIDUSER_GET_VIDEOHWND 0x1001
#define VIDUSER_SET_VFLIP 0x1002
#define VIDUSER_SET_TRACKSELINTERFACE                                          \
  0x1003 // give your ITrackSelector interface as param2
...
public:
  ...
  virtual int open(int w, int h, int vflip, double aspectratio,
                   unsigned int fmt) = 0;
  virtual void setcallback(LRESULT (*msgcallback)(void *token, HWND hwnd,
...
// these messages are callbacks that you can grab by subclassing the winamp
// window
...
#define IPC_CB_MISC_TITLE_RATING                                               \
  6 // (5.5+ for when the rating is changed via the songticker menu on current
    // file)

/* Example of using IPC_CB_MISC_STATUS to detect the start of track playback
 *with 5.x
 **
...
/* (requires Winamp 2.9+)
** int
*newpos=SendMessage(hwnd_winamp,WM_WA_IPC,(WPARAM)adjust_offset,IPC_ADJUST_FFWINDOWSMENUPOS);
** This will move where Winamp expects the freeform windows in the menubar
*windows main
** menu. This is useful if you wish to insert a menu item above extra freeform
*windows.
*/
...
/* (requires Winamp 2.9+)
** int
*newpos=SendMessage(hwnd_winamp,WM_WA_IPC,(WPARAM)adjust_offset,IPC_ADJUST_FFOPTIONSMENUPOS);
** moves where winamp expects the freeform preferences item in the menubar
*windows main
** menu. This is useful if you wish to insert a menu item above the preferences
*item.
**
** Note: This setting was ignored by gen_ff until it was fixed in 5.1
**       gen_ff would assume thatthe menu position was 11 in all cases and so
*when you
**       had two plugins attempting to add entries into the main right click
*menu it
**       would cause the 'colour themes' submenu to either be incorrectly
*duplicated or
...
** This will return the status of the time display i.e. shows time elapsed or
*remaining.
** This returns 0 if Winamp is displaying time elapsed or 1 for the time
*remaining.
*/
...
/* (requires Winamp 5.0+)
** int
*viswnd=(HWND)SendMessage(hwnd_winamp,WM_WA_IPC,(WPARAM)(HWND)viswnd,IPC_SETVISWND);
** This allows you to set a window to receive the following message commands
*(which are
** used as part of the modern skin integration).
** When you have finished or your visualisation is closed then send wParam as
*zero to
** ensure that things are correctly tidied up.
*/

/* The following messages are received as the LOWORD(wParam) of the WM_COMMAND
 *message.
 ** See %SDK%\winamp\wa5vis.txt for more info about visualisation integration in
 *Winamp.
 */
...
** This returns a HWND to the visualisation command handler window if set by
*IPC_SETVISWND.
*/
...
** This will return 1 if a visualisation is currently running and 0 if one is
*not running.
*/
...
/* (requires Winamp 5.0+)
** This is sent by Winamp back to itself so it can be trapped and adjusted as
*needed with
...
** This will return 1 if 'stop on video close' is enabled and 0 if it is
*disabled.
*/
...
** This will return a handle to the preferences dialog if it is open otherwise
*it will
** return zero. A simple check with the OS api IsWindow(..) is a good test if
*it's valid.
**
** e.g.  this will open (or close if already open) the preferences dialog and
*show if we
...
** You pass a pointer to a POINT structure which holds the width and height and
*Winamp
** will set the playlist editor to that size (this is used by gen_ff on skin
*changes).
** There does not appear to be any bounds limiting with this so it is possible
*to create
...
/* (requires Winamp 5.0+)
** HINSTANCE hInst =
*(HINSTANCE)SendMessage(hwnd_winamp,WM_WA_IPC,0,IPC_GETLANGUAGEPACKINSTANCE);
** This will return the HINSTANCE to the currently used language pack file for
*winamp.exe
**
** (5.5+)
** If you pass 1 in wParam then you will have zero returned if a language pack
*is in use.
** if(!SendMessage(hwnd_winamp,WM_WA_IPC,1,IPC_GETLANGUAGEPACKINSTANCE)){
**   // winamp is currently using a language pack
** }
**
** If you pass 2 in wParam then you will get the path to the language pack
*folder.
** wchar_t* lngpackfolder =
*(wchar_t*)SendMessage(hwnd_winamp,WM_WA_IPC,2,IPC_GETLANGUAGEPACKINSTANCE);
**
** If you pass 3 in wParam then you will get the path to the currently extracted
*language pack.
** wchar_t* lngpack =
*(wchar_t*)SendMessage(hwnd_winamp,WM_WA_IPC,3,IPC_GETLANGUAGEPACKINSTANCE);
**
** If you pass 4 in wParam then you will get the name of the currently used
*language pack.
** wchar_t* lngname =
*(char*)SendMessage(hwnd_winamp,WM_WA_IPC,4,IPC_GETLANGUAGEPACKINSTANCE);
*/
...
/*
** (5.51+)
** If you pass 5 in LOWORD(wParam) then you will get the ident string/code
*string
** (based on the param passed in the HIWORD(wParam) of the currently used
*language pack.
** The string returned with LANG_IDENT_STR is used to represent the language
*that the
** language pack is intended for following ISO naming conventions for
*consistancy.
**
** wchar_t* ident_str =
*(wchar_t*)SendMessage(hwnd_winamp,WM_WA_IPC,MAKEWPARAM(5,LANG_XXX),IPC_GETLANGUAGEPACKINSTANCE);
**
** e.g.
** For the default language it will return the following for the different
*LANG_XXX codes
...
** (you'll have to custom process the string returned if you want the langugage
*or country but that's easy ;) )
**
...
**
**
** The following example shows how using the basic api will allow you to load
*the playlist
** context menu resource from the currently loaded language pack or it will
*fallback to
...
*(HINSTANCE)SendMessage(hwnd_winamp,WM_WA_IPC,0,IPC_GETLANGUAGEPACKINSTANCE);
** HMENU popup =
*GetSubMenu(GetSubMenu((LoadMenu(lang?lang:GetModuleHandle(0),MAKEINTRESOURCE(101))),2),5);
...
**   DestroyMenu(popup);
**
** If you need a specific menu handle then look at IPC_GET_HMENU for more
*information.
*/
...
/* (requires Winamp 5.0+)
** char* outdll =
*(char*)SendMessage(hwnd_winamp,WM_WA_IPC,0,IPC_GETOUTPUTPLUGIN);
** This returns a string of the current output plugin's dll name.
** e.g. if the directsound plugin was selected then this would return
*'out_ds.dll'.
*/
...
** Set enabled to 1 to enable and 0 to disable drawing of the playlist editor
*and winamp
** gen class windows (used by gen_ff to allow it to draw its own window
*borders).
*/
...
** This will return 1 if the video or visualisation is in fullscreen mode or 0
*otherwise.
*/

#define IPC_SET_VIS_FS_FLAG 631
/* (requires Winamp 5.0+)
** A vis should send this message with 1/as param to notify winamp that it has
*gone to or has come back from fullscreen mode
*/
...
/* (requires Winamp 5.0+)
** This is a notification message sent to the main Winamp window by itself
*whenever it
** needs to get information to be shown about the current skin in the 'Current
*skin
** information' box on the main Skins page in the Winamp preferences.
**
** When this notification is received and the current skin is one you are
*providing the
** support for then you return a valid buffer for Winamp to be able to read from
*with
...
** IPC_SET_MANUALPLADVANCE sets the status of the Manual Playlist Advance
*option.
...
**
** Sent to Winamp's main window when an item has just finished playback or the
*next
...
**
** Sent to Winamp's main window when the previous button has been pressed and
*Winamp is
...
** Pass 'wnd' as an id as defined for IPC_GETWND or pass -1 to query the status
*of the
** main window. This returns 1 if the window is in winshade mode and 0 if it is
*not.
** Make sure you only test for this on a 5.04+ install otherwise you get a false
*result.
...
** This will allow you to set the 'rating' on the current playlist entry where
*'rating'
** is an integer value from 0 (no rating) to 5 (5 stars).
**
** The following example should correctly allow you to set the rating for any
*specified
** playlist entry assuming of course that you're trying to get a valid playlist
*entry.
**
...
** This returns the current playlist entry's rating between 0 (no rating) to 5
*(5 stars).
**
** The following example should correctly allow you to get the rating for any
*specified
** playlist entry assuming of course that you're trying to get a valid playlist
*entry.
**
...
** This will return the number of audio tracks available from the currently
*playing item.
*/
...
** This will return the number of video tracks available from the currently
*playing item.
*/
...
** This will return the id of the current audio track for the currently playing
*item.
*/
...
** This will return the id of the current video track for the currently playing
*item.
*/
...
** This allows you to switch to a new audio track (if supported) in the current
*playing file.
*/
...
** This allows you to switch to a new video track (if supported) in the current
*playing file.
*/
...
** This will let you disable or re-enable the UI exit functions (close button,
*context
** menu, alt-f4). Remember to call IPC_POP_DISABLE_EXIT when you are done doing
*whatever
** was required that needed to prevent exit otherwise you have to kill the
*Winamp process.
*/
...
** This will return 0 if the 'exit' option of Winamp's menu is disabled and 1
*otherwise.
*/
...
** Note: This may not match the actual TOPMOST window flag while another
*fullscreen
** application is focused if the user has the 'Disable always on top while
*fullscreen
** applications are focused' option under the  General Preferences page is
*checked.
*/
...
**
** Note: if you use this on pre 5.09 installs of Winamp then it will return 1
*which is
** not correct but is due to the way that SendMessage(..) handles un-processed
*messages.
...
** (see MSDN documentation the values passed to WinMain(..) for what this should
*be)
**
** e.g.
** if(SendMessage(hwnd_winamp,WM_WA_IPC,0,IPC_INITIAL_SHOW_STATE) ==
*SW_SHOWMINIMIZED){
**   // we are starting minimised so process as needed (keep our window hidden)
** }
**
** Useful for seeing if winamp was run minimised on startup so you can act
*accordingly.
** On pre-5.36 versions this will effectively return SW_NORMAL/SW_SHOWNORMAL due
*to the
...
//#define IPC_MBGETCURURL 1004 //copies the current URL into wParam (have a 4096
// buffer ready) #define IPC_MBGETDESC   1005 //copies the current URL
// description into wParam (have a 4096 buffer ready) #define IPC_MBCHECKLOCFILE
// 1006 //checks that the link file is up to date (otherwise updates it).
// wParam=parent HWND #define IPC_MBREFRESH   1007 //refreshes the "now playing"
// view in the library #define IPC_MBGETDEFURL 1008 //copies the default URL
// into wParam (have a 4096 buffer ready)

#define IPC_STATS_LIBRARY_ITEMCNT 1300 // updates library count status

/*
** IPC's in the message range 2000 - 3000 are reserved internally for freeform
*messages.
** These messages are taken from ff_ipc.h which is part of the Modern skin
*integration.
*/
...
/*
** This is a notification message sent when the user changes the colour theme in
*a Modern
** skin and can also be detected when the Modern skin is first loaded as the
*gen_ff plugin
...
** SendMessage(hwnd_winamp,WM_WA_IPC,(WPARAM)(const
*char*)colour_theme_name,IPC_FF_ONCOLORTHEMECHANGED);
**
** (IPC_FF_COLOURTHEME_CHANGE is the name i (DrO) was using before getting a
*copy of
...
/*
** int ismainwnd =
*(HWND)SendMessage(hwnd_winamp,WM_WA_IPC,(WPARAM)(HWND)test_wnd,IPC_FF_ISMAINWND);
**
** This allows you to determine if the window handle passed to it is a modern
*skin main
** window or not. If it is a main window or any of its windowshade variants then
*it will
** return 1.
**
** Because of the way modern skins are implemented, it is possible for this
*message to
** return a positive test result for a number of window handles within the
*current Winamp
** process. This appears to be because you can have a visible main window, a
*compact main
** window and also a winshaded version.
**
** The following code example below is one way of seeing how this api works
*since it will
** enumerate all windows related to Winamp at the time and allows you to process
*as
...
/*
** HWND wa2embed =
*(HWND)SendMessage(hwnd_winamp,WM_WA_IPC,(WPARAM)(HWND)test_wnd,IPC_FF_GETCONTENTWND);
**
** This will return the Winamp 2 window that is embedded in the window's
*container
** i.e. if hwnd is the playlist editor windowshade hwnd then it will return the
*Winamp 2
**      playlist editor hwnd.
**
** If no content is found such as the window has nothing embedded then this will
*return
...
/*
** This is a notification message sent when the user uses a global hotkey
*combination
** which had been registered with the gen_hotkeys plugin.
**
** The value of wParam is the description of the hotkey as passed to
*gen_hotkeys.
** SendMessage(hwnd_winamp,WM_WA_IPC,(WPARAM)(const
*char*)hotkey_desc,IPC_FF_NOTIFYHOTKEY);
*/
...
**
** All notification messages appear in the lParam of the main window message
*proceedure.
*/
...
/* (requires Winamp 5.0+)
** IDropTarget* IDrop =
*(IDropTarget*)SendMessage(hwnd_winamp,WM_WA_IPC,0,IPC_GETDROPTARGET);
**
** You call this to retrieve a copy of the IDropTarget interface which Winamp
*created for
** handling external drag and drop operations on to it's Windows. This is only
*really
** useful if you're providing an alternate interface and want your Windows to
*provide the
** same drag and drop support as Winamp normally provides the user. Check out
*MSDN or
** your prefered search facility for more information about the IDropTarget
*interface and
...
/* (requires Winamp 5.0+)
** This is a notification message sent to the main Winamp window whenever the
*playlist is
** modified in any way e.g. the addition/removal of a playlist entry.
**
** It is not a good idea to do large amounts of processing in this notification
*since it
** will slow down Winamp as playlist entries are modified (especially when
*you're adding
** in a large playlist).
**
...
/* (requires Winamp 5.0+)
** This is a notification message sent to the main Winamp window when playback
*begins for
...
/* (requires Winamp 5.0+)
** This is a notification message sent to the main Winamp window when a file's
*tag
** (e.g. id3) may have been updated. This appears to be sent when the
*InfoBox(..) function
** of the associated input plugin returns a 1 (which is the file information
*dialog/editor
** call normally).
**
...
/* (requires Winamp 5.0+)
** This is a notification message sent to the main Winamp window asking if it's
*okay to
** close or not. Return zero to prevent Winamp from closing or return anything
*non-zero
** to allow Winamp to close.
**
** The best implementation of this option is to let the message pass through to
*the
** original window proceedure since another plugin may want to have a say in the
*matter
...
**   visplugin_num, pe_fontsize, visplugin_priority, visplugin_autoexec,
*dspplugin_num,
**   sticon, splash, taskbar, dropaotfs, ascb_new, ttips, riol, minst,
*whichicon,
**   whichicon2, addtolist, snap, snaplen, parent, hilite, disvis, rofiob,
*shownumsinpl,
...
**
** Send write_type as 0 to write the common and less common settings and no
*playlist.
*/
...
/* (requires Winamp 5.1+)
** int (*randfunc)(void) =
*(int(*)(void))SendMessage(plugin.hwndParent,WM_WA_IPC,0,IPC_GET_RANDFUNC);
** if(randfunc && randfunc != 1){
**   randfunc();
** }
**
** This will return a positive 32-bit number (essentially 31-bit).
** The check for a returned value of 1 is because that's the default return
*value from
** SendMessage(..) when it is not handled so is good to check for it in this
*situation.
*/
...
/* (requires Winamp 5.1+)
** int
*changed=SendMessage(hwnd_winamp,WM_WA_IPC,(WPARAM)(char*)field,IPC_METADATA_CHANGED);
** a plugin can SendMessage this to winamp if internal metadata has changes.
** wParam should be a char * of what field changed
**
** Currently used for internal actions (and very few at that) the intent of this
*api is
** to allow a plugin to call it when metadata has changed in the current
*playlist entry
** e.g.a new id3v2 tag was found in a stream
**
** The wparam value can either be NULL or a pointer to an ansi string for the
*metadata
** which has changed. This can be thought of as an advanced version of
*IPC_UPDTITLE and
** could be used to allow for update of the current title when a specific tag
*changed.
**
** Not recommended to be used since there is not the complete handling
*implemented in
...
** This is a notification message sent to the main Winamp window by itself
*whenever
** the skin in use is changed. There is no information sent by the wParam for
*this.
**
** if(message == WM_WA_IPC && lparam == IPC_SKIN_CHANGED)
** {
**   // do what you need to do to handle skin changes, e.g. call
*WADlg_init(hwnd_winamp);
** }
*/
...
** This will assign you a unique id for making your own commands such as for
extra menu
** entries. The starting value returned by this message will potentially change
as and
** when the main resource file of Winamp is updated with extra data so
assumptions cannot
** be made on what will be returned and plugin loading order will affect things
as well.

** 5.33+
** If you want to reserve more than one id, you can pass the number of ids
required in wParam
*/

#define IPC_GET_DISPATCH_OBJECT                                                \
  3020 // gets winamp main IDispatch * (for embedded webpages)
#define IPC_GET_UNIQUE_DISPATCH_ID                                             \
  3021 // gives you a unique dispatch ID that won't conflict with anything in
       // winamp's IDispatch *
#define IPC_ADD_DISPATCH_OBJECT                                                \
  3022 // add your own dispatch object into winamp's.  This lets embedded
       // webpages access your functions
// pass a pointer to DispatchInfo (see below).  Winamp makes a copy of all this
// data so you can safely delete it later
typedef struct {
  wchar_t *name; // filled in by plugin, make sure it's a unicode string!! (i.e.
                 // L"myObject" instead of "myObject).
  struct IDispatch *dispatch; // filled in by plugin
  DWORD id;                   // filled in by winamp on return
} DispatchInfo;

// see IPC_JSAPI2_GET_DISPATCH_OBJECT for version 2 of the Dispatchable
// scripting interface

#define IPC_GET_PROXY_STRING 3023
/* (requires Winamp 5.11+)
** char*
*proxy_string=(char*)SendMessage(hwnd_winamp,WM_WA_IPC,0,IPC_GET_PROXY_STRING);
...
** This will return 0 if you should leave your grubby hands off the registry
*(i.e. for
** lockdown mode). This is useful if Winamp is run from a USB drive and you
*can't alter
** system settings, etc.
*/
...
*(api_service)SendMessage(hwnd_winamp,WM_WA_IPC,0,IPC_GET_API_SERVICE);
** This api will return Winamp's api_service pointer (which is what Winamp3
*used, heh).
** If this api is not supported in the Winamp version that is being used at the
*time then
** the returned value from this api will be 1 which is a good version check.
**
** As of 5.12 there is support for .w5s plugins which reside in
*%WinampDir%\System and
** are intended for common code that can be shared amongst other plugins e.g.
*jnetlib.w5s
** which contains jnetlib in one instance instead of being duplicated multiple
*times in
...
/* (requires 5.2+)
** int pl_item =
*SendMessage(hwnd_winamp,WM_WA_IPC,start,IPC_PLAYLIST_GET_NEXT_SELECTED);
**
** This works just like the ListView_GetNextItem(..) macro for ListView
*controls.
** 'start' is the index of the playlist item that you want to begin the search
*after or
** set this as -1 for the search to begin with the first item of the current
*playlist.
**
** This will return the index of the selected playlist according to the 'start'
*value or
** it returns -1 if there is no selection or no more selected items according to
*'start'.
**
** Quick example:
**
** int sel = -1;
** // keep incrementing the start of the search so we get all of the selected
*entries
** while((sel =
*SendMessage(hwnd_winamp,WM_WA_IPC,sel,IPC_PLAYLIST_GET_NEXT_SELECTED))!=-1){
...
/* (requires 5.2+)
** int selcnt =
*SendMessage(hwnd_winamp,WM_WA_IPC,0,IPC_PLAYLIST_GET_SELECTED_COUNT);
...
// send either ANSI or Unicode string (it'll figure it out, but it MUST start
// with "h"!, so don't send ftp:// or anything funny) you can also send this one
// from another process via WM_COPYDATA (unicode only)

#define IPC_USE_UXTHEME_FUNC 3033
/* (requires Winamp 5.35+)
** int ret = SendMessage(hwnd_winamp,WM_WA_IPC,param,IPC_USE_UXTHEME_FUNC);
** param can be IPC_ISWINTHEMEPRESENT or IPC_ISAEROCOMPOSITIONACTIVE or a valid
*hwnd.
**
** If you pass a hwnd then it will apply
*EnableThemeDialogTexture(ETDT_ENABLETAB)
** so your tabbed dialogs can use the correct theme (on supporting OSes ie XP+).
**
** Otherwise this will return a value based on the param passed (as defined
*below).
...
** int isthemethere =
*!SendMessage(hwnd_winamp,WM_WA_IPC,IPC_ISWINTHEMEPRESENT,IPC_USE_UXTHEME_FUNC);
*/
#define IPC_ISAEROCOMPOSITIONACTIVE 1
/* This will return 0 if aero composition is active
** int isaero =
*!SendMessage(hwnd_winamp,WM_WA_IPC,IPC_ISAEROCOMPOSITIONACTIVE,IPC_USE_UXTHEME_FUNC);
*/
...
typedef struct {
  ...
  int showCancelAll; // if set to 1, this shows a "Cancel All" button on the
                     // dialogue

  // winamp will fill these in if the call returns successfully:
  void *imgData;    // a buffer filled with compressed image data. free with
                    // WASABI_API_MEMMGR->sysFree()
...
/* pass an artFetchData*. This will show a dialog guiding the use through
 *choosing art, and return when it's finished
 ** return values:
...
** An IDispatch * object will be returned (cast the return value from
*SendMessage)
** This IDispatch can be used for scripting/automation/VB interaction
** Pass to IE via IDocHostUIHandler::GetExternal and it will become
*window.external in javscript
*/

#define IPC_REGISTER_WINAMP_IPCMESSAGE 65536
/* (requires Winamp 5.0+)
** DWORD id =
*SendMessage(hwnd_winamp,WM_WA_IPC,(WPARAM)name,IPC_REGISTER_WINAMP_IPCMESSAGE);
** The value 'id' returned is > 65536 and is incremented on subsequent calls for
*unique
** 'name' values passed to it. By using the same 'name' in different plugins
*will allow a
** common runtime api to be provided for the currently running instance of
*Winamp
** e.g.
**   PostMessage(hwnd_winamp,WM_WA_IPC,(WPARAM)my_param,registered_ipc);
** Have a look at wa_hotkeys.h for an example on how this api is used in
*practice for a
