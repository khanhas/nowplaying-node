...
#define HKF_BRING_TO_FRONT                                                     \
  0x1 // calls SetForegroundWindow before sending the message
...
#define HKF_ISPLAYING_WL                                                       \
  0x10 // sets wParam to genHotkeysAddStruct's wParam if playing, lParam if not
       // uses IPC_ISPLAYING to check if playing
#define HKF_SHOWHIDE                                                           \
  0x20 // brings Winamp to front or minimizes Winamp if already at front
...
typedef struct {
  ...
  UINT uMsg;   // message that will be sent to winamp's main window (must always
               // be !=NULL)
