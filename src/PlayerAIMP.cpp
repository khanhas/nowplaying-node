...
PlayerAIMP::PlayerAIMP()
    : Player(), m_Window(), m_WinampWindow(), m_LastCheckTime(0),
      m_LastFileSize(0), m_LastTitleSize(0), m_FileMap(), m_FileMapHandle() {}
...
bool PlayerAIMP::CheckWindow() {
  ...
  if (time - m_LastCheckTime > 5000) {
    ...
    if (m_Window) {
      ...
      m_FileMapHandle =
          OpenFileMapping(FILE_MAP_READ, FALSE, L"AIMP2_RemoteInfo");
      if (m_FileMapHandle) {
        m_FileMap =
            (LPVOID)MapViewOfFile(m_FileMapHandle, FILE_MAP_READ, 0, 0, 2048);
        if (m_FileMap) {
          m_Initialized = true;
        }
      }
...
void PlayerAIMP::UpdateData() {
  ...
  // If initialized
  m_State = (StateType)SendMessage(m_Window, WM_AIMP_COMMAND,
                                   WM_AIMP_STATUS_GET, AIMP_STS_Player);
  ...
  m_Position = (UINT)SendMessage(m_Window, WM_AIMP_COMMAND, WM_AIMP_STATUS_GET,
                                 AIMP_STS_POS);
  m_Volume = (UINT)SendMessage(m_Window, WM_AIMP_COMMAND, WM_AIMP_STATUS_GET,
                               AIMP_STS_VOLUME);
  ...
  if (info->cbSizeOf > 0 &&
    ...
    LPCWSTR stringData = (LPCWSTR)m_FileMap;
    stringData += 44;
    ...
    m_Shuffle = SendMessage(m_Window, WM_AIMP_COMMAND, WM_AIMP_STATUS_GET,
                            AIMP_STS_SHUFFLE) != 0;
    m_Repeat = SendMessage(m_Window, WM_AIMP_COMMAND, WM_AIMP_STATUS_GET,
                           AIMP_STS_REPEAT) != 0;
...
void PlayerAIMP::SetPosition(int position) {
  SendMessage(m_Window, WM_AIMP_COMMAND, WM_AIMP_STATUS_SET,
              MAKELPARAM(position, AIMP_STS_POS));
}
...
void PlayerAIMP::SetVolume(int volume) {
  SendMessage(m_Window, WM_AIMP_COMMAND, WM_AIMP_STATUS_SET,
              MAKELPARAM(volume, AIMP_STS_VOLUME));
}
...
void PlayerAIMP::SetShuffle(bool state) {
  m_Shuffle = state;
  SendMessage(m_Window, WM_AIMP_COMMAND, WM_AIMP_STATUS_SET,
              MAKELPARAM(m_Shuffle, AIMP_STS_SHUFFLE));
}
...
void PlayerAIMP::SetRepeat(bool state) {
  m_Repeat = state;
  SendMessage(m_Window, WM_AIMP_COMMAND, WM_AIMP_STATUS_SET,
              MAKELPARAM(m_Repeat, AIMP_STS_REPEAT));
}
...
**
*/
void PlayerAIMP::ClosePlayer() { SendMessage(m_Window, WM_CLOSE, 0, 0); }
...
void PlayerAIMP::OpenPlayer(std::wstring &path) {
  if (path.empty()) {
    ...
    RegOpenKeyEx(
        HKEY_LOCAL_MACHINE,
        L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\AIMP2", 0,
    ...
    if (RegQueryValueEx(hKey, L"DisplayIcon", nullptr, (LPDWORD)&type,
      ...
      RegOpenKeyEx(
          HKEY_LOCAL_MACHINE,
          L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\AIMP3", 0,
          KEY_QUERY_VALUE, &hKey);
      ...
      if (RegQueryValueEx(hKey, L"DisplayIcon", nullptr, (LPDWORD)&type,
        ...
        if (type == REG_SZ) {
          ...
          path += L"AIMP3.exe";
          ShellExecute(nullptr, L"open", path.c_str(), nullptr, nullptr,
                       SW_SHOW);
        }
      }
    }
