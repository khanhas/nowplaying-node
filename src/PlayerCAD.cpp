...
*/
PlayerCAD::PlayerCAD()
    : Player(), m_Window(), m_PlayerWindow(), m_ExtendedAPI(false),
      m_Open(false) {
  Initialize();
}
...
void PlayerCAD::Initialize() {
  ...
  m_Window =
      CreateWindow(L"NowPlayingCADClass", L"CD Art Display 1.x Class",
                   WS_DISABLED, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT,
                   CW_USEDEFAULT, nullptr, nullptr, g_Instance, this);
  ...
  auto changeWindowMessageFilterEx =
      (decltype(ChangeWindowMessageFilterEx) *)GetProcAddress(
  ...
  if (changeWindowMessageFilterEx) {
    ...
    auto changeWindowMessageFilter =
        (decltype(ChangeWindowMessageFilter) *)GetProcAddress(
  ...
  LPCWSTR file = L"D:\\datafile.ini";

  // Read saved settings
  GetPrivateProfileString(L"NowPlaying.dll", L"ClassName", nullptr, buffer,
                          MAX_PATH, file);
  std::wstring className = buffer;

  GetPrivateProfileString(L"NowPlaying.dll", L"WindowName", nullptr, buffer,
                          MAX_PATH, file);
  std::wstring windowName = buffer;

  GetPrivateProfileString(L"NowPlaying.dll", L"PlayerPath", nullptr, buffer,
                          MAX_PATH, file);
  m_PlayerPath = buffer;

  LPCWSTR classSz = className.empty() ? nullptr : className.c_str();
  LPCWSTR windowSz = windowName.empty() ? nullptr : windowName.c_str();
  ...
  if (classSz || windowSz) {
    ...
    if (m_PlayerWindow) {
      ...
      windowSz = (GetWindowText(m_PlayerWindow, buffer, MAX_PATH) > 0)
                     ? buffer
                     : nullptr;
      WritePrivateProfileString(L"NowPlaying.dll", L"WindowName", windowSz,
                                file);
      ...
      if (hProcess) {
        if (GetModuleFileNameEx(hProcess, nullptr, buffer, MAX_PATH) > 0) {
          WritePrivateProfileString(L"NowPlaying.dll", L"PlayerPath", buffer,
                                    file);
        }
  ...
  if (m_PlayerWindow) {
    ...
    SendMessage(m_PlayerWindow, WM_USER, (WPARAM)m_Window,
...
**
*/
LRESULT CALLBACK PlayerCAD::WndProc(HWND hwnd, UINT msg, WPARAM wParam,
                                    LPARAM lParam) {
  ...
  case WM_COPYDATA: {
    ...
    if (cds->dwData == IPC_CURRENT_TRACK_NOTIFICATION) {
      player->m_Shuffle =
          SendMessage(player->m_PlayerWindow, WM_USER, 0, IPC_GET_SHUFFLE) != 0;
      player->m_Repeat =
          SendMessage(player->m_PlayerWindow, WM_USER, 0, IPC_GET_REPEAT) != 0;
    ...
    } else if (cds->dwData == IPC_REGISTER_NOTIFICATION &&
               !player->m_Initialized) {
      ...
      if (data[0] == L'1') {
        ...
        WritePrivateProfileString(L"NowPlaying.dll", L"ClassName", classSz,
                                  file);
        WritePrivateProfileString(L"NowPlaying.dll", L"WindowName", windowSz,
                                  file);
        WritePrivateProfileString(L"NowPlaying.dll", L"PlayerPath",
                                  player->m_PlayerPath.c_str(), file);

        player->m_PlayerWindow = FindWindow(classSz, windowSz);

        if (player->m_PlayerWindow) {
          player->m_Initialized = true;
          player->m_ExtendedAPI =
              (classSz && wcscmp(classSz, L"CD Art Display IPC Class") == 0);
          player->m_State = (StateType)SendMessage(player->m_PlayerWindow,
                                                   WM_USER, 0, IPC_GET_STATE);

          if (player->m_State != STATE_STOPPED) {
            PostMessage(player->m_PlayerWindow, WM_USER, 0,
...
void PlayerCAD::UpdateData() {
  if (m_State != STATE_STOPPED) {
    m_Position =
        (UINT)SendMessage(m_PlayerWindow, WM_USER, 0, IPC_GET_POSITION);
...
void PlayerCAD::Pause() {
  SendMessage(m_PlayerWindow, WM_USER, 0,
              m_ExtendedAPI ? IPC_PAUSE : IPC_PLAYPAUSE);
}
...
void PlayerCAD::Play() {
  SendMessage(m_PlayerWindow, WM_USER, 0,
              m_ExtendedAPI ? IPC_PLAY : IPC_PLAYPAUSE);
}
...
**
*/
void PlayerCAD::Stop() { SendMessage(m_PlayerWindow, WM_USER, 0, IPC_STOP); }

/*
** Handles the Next bang.
**
*/
void PlayerCAD::Next() { SendMessage(m_PlayerWindow, WM_USER, 0, IPC_NEXT); }
...
void PlayerCAD::OpenPlayer(std::wstring &path) {
  if (!m_Initialized) {
    ...
    if (!path.empty()) {
      ret = ShellExecute(nullptr, L"open", path.c_str(), nullptr, nullptr,
                         SW_SHOW);
    } else if (!m_PlayerPath.empty()) {
      ret = ShellExecute(nullptr, L"open", m_PlayerPath.c_str(), nullptr,
                         nullptr, SW_SHOW);
    }
