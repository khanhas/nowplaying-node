...
bool WriteCoverToFile(const TagLib::ByteVector &data,
...
**
*/
bool CCover::GetLocal(std::wstring filename, const std::wstring &folder,
                      std::wstring &target) {
  ...
  LPCWSTR extName[extCount] = {L"jpg", L"jpeg", L"png", L"bmp"};
...
**
*/
bool CCover::GetEmbedded(const TagLib::FileRef &fr,
                         const std::wstring &target) {
  bool found = false;
  ...
  if (TagLib::MPEG::File *file =
    ...
    if (!found && file->ID3v2Tag()) {
      found = ExtractID3(file->ID3v2Tag(), target);
                 ...
                 dynamic_cast<TagLib::MP4::File *>(fr.file())) {
    found = ExtractMP4(file, target);
  } else if (TagLib::ASF::File *file =
                 dynamic_cast<TagLib::ASF::File *>(fr.file())) {
    found = ExtractASF(file, target);
  } else if (TagLib::APE::File *file =
                 dynamic_cast<TagLib::APE::File *>(fr.file())) {
    if (file->APETag()) {
      found = ExtractAPE(file->APETag(), target);
    }
  } else if (TagLib::MPC::File *file =
                 dynamic_cast<TagLib::MPC::File *>(fr.file())) {
    if (file->APETag()) {
      found = ExtractAPE(file->APETag(), target);
    }
  }
