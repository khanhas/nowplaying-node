...
PlayerITunes::CEventHandler::CEventHandler(PlayerITunes *player)
    : m_Player(player), m_RefCount(0UL), m_ConnectionPoint(),
      m_ConnectionCookie(0UL) {
  IConnectionPointContainer *icpc;
  m_Player->m_iTunes->QueryInterface(IID_IConnectionPointContainer,
                                     (void **)&icpc);
...
PlayerITunes::CEventHandler::QueryInterface(REFIID iid, void **ppvObject) {
  if (iid == IID_IUnknown || iid == IID_IUnknown ||
      iid == DIID__IiTunesEvents) {
    ++m_RefCount;
    *ppvObject = this;
    return S_OK;
  }
...
PlayerITunes::PlayerITunes()
    : Player(), m_CallbackWindow(), m_LastCheckTime(0UL), m_iTunesActive(false),
      m_iTunes(), m_iTunesEvent() {
  ...
  m_CallbackWindow =
      CreateWindow(L"NowPlayingITunesClass", L"CallbackWindow", WS_DISABLED,
                   CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT,
                   HWND_MESSAGE, nullptr, g_Instance, this);
}
...
void PlayerITunes::Initialize() {
  while (true) {
    HRESULT hr = CoCreateInstance(CLSID_iTunesApp, nullptr, CLSCTX_LOCAL_SERVER,
                                  IID_IiTunes, (PVOID *)&m_iTunes);
...
**
*/
LRESULT CALLBACK PlayerITunes::WndProc(HWND hwnd, UINT msg, WPARAM wParam,
                                       LPARAM lParam) {
...
bool PlayerITunes::CheckWindow() {
  ...
  if (time - m_LastCheckTime > 500UL) {
    ...
    if ((FindWindow(L"iTunesApp", L"iTunes") ||
         FindWindow(L"iTunes", L"iTunes"))) {
      m_iTunesActive = true;
      if (!m_Initialized) {
        Initialize();
      }
...
**
*/
void PlayerITunes::Pause() { m_iTunes->Pause(); }

/*
** Handles the Play bang.
**
*/
void PlayerITunes::Play() { m_iTunes->Play(); }

/*
** Handles the Stop bang.
**
*/
void PlayerITunes::Stop() { m_iTunes->Stop(); }

/*
** Handles the Next bang.
**
*/
void PlayerITunes::Next() { m_iTunes->NextTrack(); }

/*
** Handles the Previous bang.
**
*/
void PlayerITunes::Previous() { m_iTunes->PreviousTrack(); }
...
void PlayerITunes::OpenPlayer(std::wstring &path) {
  ShellExecute(nullptr, L"open", path.empty() ? L"iTunes.exe" : path.c_str(),
