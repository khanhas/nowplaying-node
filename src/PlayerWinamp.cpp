...
**
*/
PlayerWinamp::PlayerWinamp(WINAMPTYPE type)
    : Player(), m_Window(), m_LastCheckTime(0), m_UseUnicodeAPI(false),
      m_PlayingStream(false), m_WinampType(type), m_WinampHandle(),
      m_WinampAddress() {}
...
bool PlayerWinamp::CheckWindow() {
  ...
  if (time - m_LastCheckTime > 5000) {
    ...
    if (m_Window) {
      ...
      if (m_WinampHandle) {
        m_WinampAddress = (LPCVOID)SendMessage(m_Window, WM_WA_IPC, 0,
...
void PlayerWinamp::UpdateData() {
  if (m_Initialized || CheckWindow()) {
    ...
    if (playing == 0) {
      ...
      m_Position =
          (UINT)SendMessage(m_Window, WM_WA_IPC, 0, IPC_GETOUTPUTTIME) /
          1000; // ms to secs
      m_Volume =
          ((UINT)SendMessage(m_Window, WM_WA_IPC, -666, IPC_SETVOLUME) * 100) /
          255; // 0 - 255 to 0 - 100
    }
    ...
    if (m_UseUnicodeAPI) {
      if (!ReadProcessMemory(m_WinampHandle, m_WinampAddress, &wBuffer,
                             sizeof(wBuffer), nullptr)) {
      ...
      LPCVOID address =
          (LPCVOID)SendMessage(m_Window, WM_WA_IPC, pos, IPC_GETPLAYLISTFILE);

      if (!ReadProcessMemory(m_WinampHandle, address, &cBuffer, sizeof(cBuffer),
                             nullptr)) {
        // Failed to read memory
        return;
      }
    ...
    if (wcscmp(wBuffer, m_FilePath.c_str()) != 0) {
      ...
      if (!m_PlayingStream) {
        const int duration =
            (int)SendMessage(m_Window, WM_WA_IPC, 1, IPC_GETOUTPUTTIME);
    ...
    // TagLib couldn't parse the file or Winamp is playing a stream, try to get
    // title
    if (m_UseUnicodeAPI) {
      LPCVOID address =
          (LPCVOID)SendMessage(m_Window, WM_WA_IPC, 0, IPC_GET_PLAYING_TITLE);
      ReadProcessMemory(m_WinampHandle, address, &wBuffer, sizeof(wBuffer),
                        nullptr);
    } else {
      const auto pos = SendMessage(m_Window, WM_WA_IPC, 0, IPC_GETLISTPOS);
      LPCVOID address =
          (LPCVOID)SendMessage(m_Window, WM_WA_IPC, pos, IPC_GETPLAYLISTTITLE);
      ReadProcessMemory(m_WinampHandle, address, &cBuffer, sizeof(cBuffer),
                        nullptr);
      mbstowcs(wBuffer, cBuffer, MAX_PATH);
    }
...
**
*/
void PlayerWinamp::Play() { SendMessage(m_Window, WM_COMMAND, WINAMP_PLAY, 0); }

/*
** Handles the Stop bang.
**
*/
void PlayerWinamp::Stop() { SendMessage(m_Window, WM_COMMAND, WINAMP_STOP, 0); }
...
void PlayerWinamp::OpenPlayer(std::wstring &path) {
  if (m_WinampType == WA_WINAMP) {
    ShellExecute(nullptr, L"open", path.empty() ? L"winamp.exe" : path.c_str(),
  ...
  } else // if (m_WinampType == WA_MEDIAMONKEY)
  {
    if (path.empty()) {
      ...
      RegOpenKeyEx(
          HKEY_LOCAL_MACHINE,
          L"SOFTWARE\\Clients\\Media\\MediaMonkey\\shell\\open\\command", 0,
      ...
      if (RegQueryValueEx(hKey, nullptr, nullptr, (LPDWORD)&type, (LPBYTE)data,
