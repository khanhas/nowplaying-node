...
// Definitions of ATL stuff we need to avoid dependency on atlbase.h (which is
// not included in free versions of Visual Studio).
...
public:
  ...
  virtual HRESULT STDMETHODCALLTYPE CreateControlEx(
      LPCOLESTR lpszTricsData, HWND hWnd, IStream * pStream, IUnknown * *ppUnk,
      REFIID iidAdvise, IUnknown * punkSink) = 0;

  virtual HRESULT STDMETHODCALLTYPE AttachControl(IUnknown * pUnkControl,
                                                  HWND hWnd) = 0;
  virtual HRESULT STDMETHODCALLTYPE QueryControl(REFIID riid,
                                                 void **ppvObject) = 0;
...
PlayerWMP::CRemoteHost::CRemoteHost() : m_Player(), m_RefCount(0) {}
...
PlayerWMP::CRemoteHost::QueryInterface(IID const &riid, void **object) {
  if (!object) {
    return E_POINTER;
  }

  if (riid == __uuidof(IUnknown) || riid == __uuidof(IServiceProvider)) {
...
HRESULT PlayerWMP::CRemoteHost::QueryService(REFGUID guidService, REFIID riid,
                                             void **ppv) {
...
HRESULT PlayerWMP::CRemoteHost::GetScriptableObject(BSTR *pbstrName,
...
PlayerWMP::PlayerWMP()
    : Player(), m_TrackChanged(false), m_Window(), m_LastCheckTime(0),
      m_ConnectionCookie() {}
...
void PlayerWMP::Initialize() {
  ...
  auto atlAxGetControl =
      (AtlAxGetControlFunc)GetProcAddress(atl, "AtlAxGetControl");
  ...
  m_Window = CreateWindow(L"NowPlayingWMP", L"", WS_DISABLED, CW_USEDEFAULT,
  ...
  HWND window = CreateWindow(
      ...
      CW_USEDEFAULT, CW_USEDEFAULT, m_Window, nullptr, g_Instance, nullptr);
  ...
  if (SUCCEEDED(hr)) {
    hr = axWinHostWindow->CreateControl(
        L"{6BF52A52-394A-11D3-B153-00C04F79FAA6}", window, nullptr);
  }
  ...
  if (SUCCEEDED(hr)) {
    hr = m_IConnectionPoint->Advise(remoteHost->GetUnknown(),
                                    &m_ConnectionCookie);
...
**
*/
void PlayerWMP::Pause() { m_IControls->pause(); }

/*
** Handles the Play bang.
**
*/
void PlayerWMP::Play() { m_IControls->play(); }
...
**
*/
void PlayerWMP::Next() { m_IControls->next(); }

/*
** Handles the Previous bang.
**
*/
void PlayerWMP::Previous() { m_IControls->previous(); }
...
**
*/
void PlayerWMP::SetVolume(int volume) { m_ISettings->put_volume(volume); }
...
void PlayerWMP::OpenPlayer(std::wstring &path) {
  ShellExecute(nullptr, L"open", path.empty() ? L"wmplayer.exe" : path.c_str(),
